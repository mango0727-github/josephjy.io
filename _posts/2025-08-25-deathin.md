...
![Figure 1. Malware analysis result on PEStudio](image.png)
...
![Figure 2. Ghidra_GolangAnalyzerExtension GitHub main screen](image%201.png)

Figure 2. Ghidra_GolangAnalyzerExtension GitHub main screen

![Figure 3. Install Extensions menu in the Ghidra main console](image%202.png)

Figure 3. Install Extensions menu in the Ghidra main console

![Figure 4. Popup window when selecting Install Extensions](image%203.png)

Figure 4. Popup window when selecting Install Extensions

![Figure 5. How to analyze with the extension after installing and restarting the extension.](image%204.png)

Figure 5. How to analyze with the extension after installing and restarting the extension.

![Figure 6. Restored result with Goresym alone](image%205.png)
...
![Figure 7. Pre-hardcoded error window message on Ghidra CodeBrowser](image%206.png)

Figure 7. Pre-hardcoded error window message on Ghidra CodeBrowser

![Figure 8. fake error warning window displayed when running the fakeerror function](image%207.png)
...
![Figure 9. Subroutines for anti-debugging in the antidebug.Run function in Ghidra CodeBrowser](image%208.png)
...
![Figure 10. Username checking routine in the isBlackListedUser function in Ghidra CodeBrowser](image%209.png)
...
![Figure 11. Computer name checking routine in the isBlackListedPC function on Ghidra CodeBrowser](image%2010.png)
...
![Figure 12. Routine in the isBlackListedHWID function on Ghidra CodeBrowser](image%2011.png)
...
![Figure 13. Routine for checking public IP and MAC address on Ghidra CodeBrowser](image%2012.png)

Figure 13. Routine for checking public IP and MAC address on Ghidra CodeBrowser

![Figure 14. Ghidra CodeBrowser blacklist IP checking routine](image%2013.png)

Figure 14. Ghidra CodeBrowser blacklist IP checking routine

![Figure 15. Routine for checking MAC addresses on Ghidra CodeBrowser](image%2014.png)
...
![Figure 16. Screen Size Check Routine on Ghidra CodeBrowser](image%2015.png)
...
![Figure 17. Memory size check routine on Ghidra CodeBrowser](image%2016.png)
...
![Figure 21. Go routines on Ghidra CodeBrowser](image%2020.png)
...
![Figure 22. BetterDiscord and DiscordTokenProtector bypass routine in injection.Run function on Ghidra CodeBrowser](image%2021.png)
...
![Figure 23. Checking the Discord directory and injectDiscord routine in the injection.Run function on Ghidra CodeBrowser](image%2022.png)

Figure 23. Checking the Discord directory and injectDiscord routine in the injection.Run function on Ghidra CodeBrowser

![Figure 24. Discord directory on x64dbg](image%2023.png)
...
![Figure 25. Routines within the injectDiscord function on Ghidra CodeBrowser](image%2024.png)

Figure 25. Routines within the injectDiscord function on Ghidra CodeBrowser

![Figure 26. Traversing the discord_desktop_core folder by the getCore function on x64dbg](image%2025.png)

Figure 26. Traversing the discord_desktop_core folder by the getCore function on x64dbg

![Figure 27. Injection target file (index.js) inside the discord_desktop_core folder](image%2026.png)
...
![Figure 28. host_url, the third argument of net/http.(*Client).Get() function with no data on Ghidra CodeBrowser](image%2027.png)

Figure 28. host_url, the third argument of net/http.(*Client).Get() function with no data on Ghidra CodeBrowser

![Figure 29. RCX (host_url) is zeroed out even between dynamic analysis on x64dbg](image%2028.png)
...
![Figure 30. Routine in discords.Run function for collecting data on Ghidra CodeBrowser](image%2029.png)

Figure 30. Routine in discords.Run function for collecting data on Ghidra CodeBrowser

![Figure 31. List of identified browsable folders on x64dbg](image%2030.png)

Figure 31. List of identified browsable folders on x64dbg

![Figure 32. Identified browse target file on x64dbg: discord_backup_codes](image%2031.png)

Figure 32. Identified browse target file on x64dbg: discord_backup_codes

![Figure 33. Routine for generating data in json file format for sending webhooks on Ghidra CodeBrowser](image%2032.png)

Figure 33. Routine for generating data in json file format for sending webhooks on Ghidra CodeBrowser

![Figure 34. Discord webhook: Sending data as HTTP POST request to Discord address (hxxps[://]discord[.]com/api/webhooks/1101151106052145214/BIaHrwzWkurP1ifNTfI0S-nV_adpU3L7CtHkZgsoxNh0xWIhQpjX2fdzD9kB7BDNYQi7)](image%2033.png)
...
![Figure 35. Ghidra CodeBrowser's browser.Run function's Gecko and Chromium-based browser master key capture, information collection, and file compression routines.](image%2034.png)
...
![Figure 36. Information gathering routine in chromium.Steal() function on Ghidra CodeBrowser](image%2035.png)
...
![Figure 37. Local State navigation routine on Ghidra CodeBrowser](image%2036.png)

Figure 37. Local State navigation routine on Ghidra CodeBrowser

![Figure 38. Local State contents (partially obscured for security reasons)](image%2037.png)

Figure 38. Local State contents (partially obscured for security reasons)

![Figure 39. Masterkey base64 decoding and DPAPI decryption routine on Ghidra CodeBrowser](image%2038.png)
...
![Figure 40. Routine to collect information only for the Default profile on Ghidra CodeBrowser. Parsing the condition of the If statement results in Opera or OperaGX.](image%2039.png)
...
![Figure 41. Routine to collect information for all profiles on Ghidra CodeBrowser. Note Current_profile + 2.](image%2040.png)
...
![Figure 42. Ghidra CodeBrowser creates directories and text files based on the collected information, and compresses the files.](image%2041.png)
...
![Figure 43. Dropped browsers folder and browser.zip](image%2042.png)

Figure 43. Dropped browsers folder and browser.zip

![Figure 44. Chrome and Edge folders present in the browsers folder](image%2043.png)

Figure 44. Chrome and Edge folders present in the browsers folder

![Figure 45. text file with downloads, history information collected from the Default profile of the dropped Chrome browser](image%2044.png)

Figure 45. text file with downloads, history information collected from the Default profile of the dropped Chrome browser

![Figure 46. Routines to generate json format data, send webhooks, and delete files on Ghidra CodeBrowser](image%2045.png)
...
![Figure 47. Various data collection functions in the system.Run function on Ghidra CodeBrowser](image%2046.png)
...
![Figure 48. Display information collection and screenshot capture routine on Ghidra CodeBrowser](image%2047.png)

Figure 48. Display information collection and screenshot capture routine on Ghidra CodeBrowser

![Figure 49. Dropped screenshot png file](image%2048.png)

Figure 49. Dropped screenshot png file

![Figure 50. Contents of the dropped screenshot file (0_1920x1080.png)](image%2049.png)
...
![Figure 51. Host and User name collection routine on Ghidra CodeBrowser](image%2050.png)
...
![Figure 52. Ghidra CodeBrowser HWID, CPU, Virtual memory, GPU information collection routine](image%2051.png)
...
![Figure 53. Ghidra CodeBrowser Public IP and MAC Information Collection Routine](image%2052.png)

Figure 53. Ghidra CodeBrowser Public IP and MAC Information Collection Routine

![Figure 54. Routine to collect country, city, region, zip code, etc. information on Ghidra CodeBrowser](image%2053.png)
...
![Figure 55. Product key and product name collection routine using powershell on Ghidra CodeBrowser](image%2054.png)

Figure 55. Product key and product name collection routine using powershell on Ghidra CodeBrowser

![Figure 56. Identified product key on x64dbg](image%2055.png)

Figure 56. Identified product key on x64dbg

![Figure 57. Identified product name on x64dbg](image%2056.png)
...
![Figure 58. Routine for sending webhook and deleting png file on Ghidra CodeBrowser](image%2057.png)
...
![Figure 59. Routine to explore User Data\Local State or Local Data\Local State on Ghidra CodeBrowser](image%2058.png)
...
![Figure 60. Routine to browse .log or .ldb file on Ghidra CodeBrowser](image%2059.png)
...
![Figure 61. cord string detection routine from read data on Ghidra CodeBrowser](image%2060.png)

Figure 61. cord string detection routine from read data on Ghidra CodeBrowser

![Figure 62. Searching for a disco token using a regular expression ([\\w-]{26}\\.[\\w-]{6}\\.[\\w-]{25,110}|mfa\\.[\\w-]{80,95}) in the absence of a string](image%2061.png)
...
![Figure 63. x64dbg-identified token detection regex when no cord string is present](image%2062.png)

Figure 63. x64dbg-identified token detection regex when no cord string is present

![Figure 64. Detecting a discovery token utilizing a regex (dQw4w9WgXcQ:[^\\\"]) when the string is present](image%2063.png)

Figure 64. Detecting a discovery token utilizing a regex (dQw4w9WgXcQ:[^\\\"]) when the string is present

![Figure 65. x64dbg-identified token detection regular expression (when cord string is present)](image%2064.png)

Figure 65. x64dbg-identified token detection regular expression (when cord string is present)

![Figure 66. Base64 decoding and decryption routine for token identified by Ghidra CodeBrowser](image%2065.png)
...
![Figure 67. Routine for sending HTTP GET request with token to hxxps[://]discord[.]com/api/v9/users/@me in Ghidra CodeBrowser](image%2066.png)

Figure 67. Routine for sending HTTP GET request with token to hxxps[://]discord[.]com/api/v9/users/@me in Ghidra CodeBrowser

![Figure 68. User Structure of the User Object, from [https://discord.com/developers/docs/resources/user#user-object](https://discord.com/developers/docs/resources/user#user-object)](image%2067.png)
...
![Figure 69. User Flags in User Object, from [https://discord.com/developers/docs/resources/user#user-object](https://discord.com/developers/docs/resources/user#user-object)](image%2068.png)
...
![Figure 70. Premium Types in User Object, from [https://discord.com/developers/docs/resources/user#user-object](https://discord.com/developers/docs/resources/user#user-object)](image%2069.png)
...
![Figure 71. Routine to get payment information in Ghidra CodeBrowser](image%2070.png)
...
![Figure 72. Routine to get guild information in Ghidra CodeBrowser](image%2071.png)
...
![Figure 73. Routine to get friend information in Ghidra CodeBrowser](image%2072.png)
...
![Figure 74. Routine to get avatar information in Ghidra CodeBrowser](image%2073.png)
...
![Figure 75. Routine to create the files directory in Ghidra CodeBrowser](image%2074.png)

Figure 75. Routine to create the files directory in Ghidra CodeBrowser

![Figure 76. Dropped files folder in the working directory (desktop)](image%2075.png)
...
![Figure 77. List of target folders for file browsing identified on x64dbg](image%2076.png)
...
![Figure 78. Routine to copy files with target extensions to the files folder on Ghidra CodeBrowser](image%2077.png)

Figure 78. Routine to copy files with target extensions to the files folder on Ghidra CodeBrowser

![Figure 79. Files with the dropped target extension in the files folder](image%2078.png)
...
![Figure 80. Routine to compress the files folder into a zip file on Ghidra CodeBrowser](image%2079.png)

Figure 80. Routine to compress the files folder into a zip file on Ghidra CodeBrowser

![Figure 81. files.zip dropped on the working directory (desktop)](image%2080.png)
...
![Figure 82. Executing the GoFileUpload function with files.zip as an argument in Ghidra CodeBrowser](image%2081.png)

Figure 82. Executing the GoFileUpload function with files.zip as an argument in Ghidra CodeBrowser

![Figure 83. Routine for sending an HTTP GET request to api[.]gofile[.]io/getServer on Ghidra CodeBrowser and processing the response](image%2082.png)
...
![Figure 84. Description of api[.]gofile[.]io/servers in the gofile API documentation, from [https://gofile.io/api.](https://gofile.io/api)](image%2083.png)
...
![Figure 85. Routine for uploading zip file via POST request to Gofile on Ghidra CodeBrowser](image%2084.png)

Figure 85. Routine for uploading zip file via POST request to Gofile on Ghidra CodeBrowser

![Figure 86. Response packet structure for POST request to Go file server](image%2085.png)
...
![Figure 87. Writing transfer data in json format including download link](image%2086.png)

Figure 87. Writing transfer data in json format including download link

![Figure 88. Webhook sending routine on Ghidra CodeBrowser](image%2087.png)
...
![Figure 89. The clipboard.watch function inside the clipper.Run function on Ghidra CodeBrowser.](image%2088.png)
...
![Figure 90. The clipboard.watch function routine on Ghidra CodeBrowser, running watch.func1 and receiving data through the channel.](image%2089.png)
...
![Figure 91. Calling Windows API, GetClipboardSenquenceNumber, on Ghidra CodeBrowser.](image%2090.png)

Figure 91. Calling Windows API, GetClipboardSenquenceNumber, on Ghidra CodeBrowser.

![Figure 92. Windows API function GetClipboardSequenceNumber identified on x64dbg.](image%2091.png)
...
![Figure 93. The clipboard read routine in the clipboard.watch.func1 function on Ghidra CodeBrowser.](image%2092.png)
...
![Figure 94. Data traversal routine through regular expressions based on clipboard changes on Ghidra CodeBrowser.](image%2093.png)
...
![Figure 95. Routine to overwrite the clipboard with the attacker's desired data on Ghidra CodeBrowser](image%2094.png)
